...
int main()
{
    ...
    { // Sum up elements of a sequence, starting from the value 0.0.
        const auto v = vector<double>{ 1.5, 2.7, 3.8, 4.2 };
                               //v converted to range structer then operations are made
                               //buraya girilen type cinsinde değer return eder ve ona göre hesaplar( v,0 -> 10),
        auto val = accumulate(v,0); // 12.2
        std::cout << val << std::endl;
    }

    { // Fold elements of a sequence via multiplication, starting from the value 1.
        const auto v = vector<int>{ 1, 2, 3, 4 };
                                    // accumulate algorithm can be used with a function object
        auto val = accumulate(v, 1, std::multiplies<int>()); // 24
        std::cout << val << std::endl;
    }
    ...
    { // Determine equality of two vectors. [1, 2, 3] == [3, 2, 1]
       ...
       { // Calculate the dot product of two vectors.
           ...
           const auto v2 = vector<int>{ 4, -2, -1};
                                           //accumulates, it understands type will be integer
       ...
       { // Create a vector of tuples, i.e. a cartesian product of two sequences.
           ...
           for(auto [a, b] : rng)
               std::cout << a << ' ' << b << ", "; // A 1, A 2, B 1, B 2, C 1, C 2
           std::cout << std::endl;
       }

       { // Create a sequence of powers of two: 1, 2, 4, 8, 16, ..., goes to infinity
       ...
       {
           v1 |= actions::sort;  // v1 = v1 | actions::sort;
           for(auto a : v1)
               std::cout << a << std::endl;
       }
       new_line();
