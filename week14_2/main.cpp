...
//std::plus<float>() ---> [](float a, float b){ return a + b;};
//std::minus<float>() ---> [](float a, float b){ return a - b;};
//std::multiplies<float>() ---> [](float a, float b){ return a * b;};
...
int main()
{
    ...
    { // any_of, all_of, none_of
        auto v = vector<int>{6, 2, 3, 4, 5, 6};
        cout << std::boolalpha; // 0,1 yerine false, true bastırtmak için
    ...
    { // for_each
        ...
        cout << views::all(v) << endl; // [1,2,3,4,5,6] olarak print eder
...

    {
            ...
            for_each(v,[](int& a){a+=10;});
            cout << (v | views::all) << endl;
    }
